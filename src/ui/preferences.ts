...
export class PreferencesUI {
  ...
  constructor(
    ...
    lifecycle.add(runtime.hook("PreferenceRun", 5, (args, next) => { const result = next(args); this.drawPreferenceEntry(); return result; }));
    lifecycle.add(runtime.hook("PreferenceClick", 5, (args, next) => { if (typeof MouseIn === "function" && MouseIn(1540, 880, 410, 80)) { this.open(); return; } return next(args); }));
    ...
    lifecycle.interval(() => this.ensurePreferenceDomEntry(), 700);
    lifecycle.add(() => { this.close(); this.entryButton?.remove(); });
  }
  ...
  private renderHypnosis(): string {
    ...
    return `<div class="grid two">
      ...
      ${control("HUD", checkbox("general.showHud", s.general.showHud), "Show depth and emergency controls in the room.")}
    </div>`;
  }
  ...
  private renderSuggestions(): string {
    ...
    return `<div class="split"><div class="panel list"><button data-action="suggestion-add">+ New suggestion</button>${list.map((item) => `<button data-action="suggestion-select" data-value="${attr(item.id)}" class="${item.id === selected?.id ? "active" : ""}"><b>${escapeHtml(item.name)}</b><small>${escapeHtml(item.trigger)}</small></button>`).join("") || `<p>No suggestions installed.</p>`}</div><div>${selected ? this.renderSuggestionEditor(selected) : `<div class="panel"><h3>Create your first suggestion</h3><p>Suggestions combine trigger phrases, effects, sounds, conditions and BC actions.</p></div>`}</div></div>`;
  }
  ...
  private onClick(event: Event): void {
    ...
    switch (action) {
      ...
      case "depth": this.hypno.addDepth(Number(value), "preference control"); break;
      case "trance": this.hypno.enterTrance(this.runtime.memberNumber, "preference control"); break;
      case "wake": this.hypno.wake("preference control"); break;
  ...
  private drawPreferenceEntry(): void {
    if (typeof DrawButton === "function") DrawButton(1540, 880, 410, 80, "SkyzHypno", "#ff58bd", "", "Open SkyzHypno preferences");
  }
  ...
  private ensurePreferenceDomEntry(): void {
    const isPreference = typeof CurrentScreen !== "undefined" && /Preference/i.test(CurrentScreen);
    if (!isPreference) { this.entryButton?.remove(); this.entryButton = undefined; return; }
    if (this.entryButton?.isConnected) return;
    const button = document.createElement("button"); button.textContent = "SkyzHypno"; button.style.cssText = "position:fixed;right:18px;bottom:18px;z-index:99989;padding:10px 16px;border:1px solid #ff58bd;border-radius:14px;background:#15091fcc;color:white;box-shadow:0 0 24px #ff58bd55;cursor:pointer"; button.addEventListener("click", () => this.open()); document.body.appendChild(button); this.entryButton = button;
  }
}

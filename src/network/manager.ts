...
const settingsPatchPayload = z.object({ changes: z.object({ autoWakeMinutes: z.number().min(0).max(1440).optional(), lucidTrance: z.boolean().optional(), effectIntensity: z.number().min(0).max(1).optional(), themeMode: z.enum(["pinky","dark","hybrid"]).optional(), restrictionThreshold: z.number().min(0).max(100).optional() }).strict() }).strict();
...
export class NetworkManager {
  ...
  requestSettingsPatch(target: number, changes: { autoWakeMinutes?: number; lucidTrance?: boolean; effectIntensity?: number; themeMode?: "pinky" | "dark" | "hybrid"; restrictionThreshold?: number }): void { this.send("settings.patch", { changes }, target); }
...
const publicStatusSchema = z.object({
  ...
  activeBy: z.number().int().positive().optional(), capabilities: z.array(z.enum(["viewStatus","trigger","deepen","wake","testEffect","installSuggestion","editOwnSuggestion","removeOwnSuggestion","editSettings","startSession","controlSession"])).max(30), theme: z.enum(["pinky", "dark", "hybrid", "custom"]),

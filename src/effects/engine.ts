...
export class EffectsEngine {
  ...
  private mount(): void {
    ...
    root.innerHTML = `<canvas id="sh-effect-canvas"></canvas><div class="sh-blur"></div><div class="sh-tint"></div><div class="sh-room-aura"></div><div class="sh-vignette"></div><div class="sh-tunnel"></div><div class="sh-waves"></div><div class="sh-dream"></div><div class="sh-glitch"></div><div class="sh-flash"></div>`;
    document.body.appendChild(root);
    const canvas = root.querySelector<HTMLCanvasElement>("#sh-effect-canvas")!;
    this.root = root; this.canvas = canvas; this.ctx = canvas.getContext("2d") ?? undefined;
    this.resize();
  ...
  private effectiveIntensity(): number {
    ...
    const hasPreview = this.activePresets.length > 0;
    const hasAftereffects = Boolean(this.state.aftereffectsUntil && this.state.aftereffectsUntil > Date.now());
    if (this.state.depth <= 0 && !hasPreview && !hasAftereffects) return 0;
    const depthScale = hasPreview ? Math.max(0.35, this.state.depth / 100) : hasAftereffects ? 0.22 : this.state.depth / 100;
    return Math.max(0, Math.min(1, effects.intensity * depthScale));
  }
  ...
  private updateDom(): void {
    ...
    this.root.style.display = settings.general.enabled && settings.effects.enabled && intensity > 0.001 ? "" : "none";
  ...
  private drawCharacterHook(args: any[], next: (args: any[]) => any): any {
    ...
    const effects = this.effectiveEffects();
    const canDecorate = typeof MainCanvas !== "undefined" && character?.MemberNumber && Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(zoom);
    const isFocus = Boolean(canDecorate && focus && character.MemberNumber === focus && effects.focusLock);

    if (canDecorate && focus && effects.focusLock) {
      MainCanvas.save();
    ...
    if (canDecorate && this.shouldDrawStatusIndicator(character)) this.drawStatusIndicator(x, y, zoom);
    return result;
  }

  private shouldDrawStatusIndicator(character: Character): boolean {
    const settings = this.store.value;
    const isPlayer = character.MemberNumber === this.runtime.memberNumber || character.IsPlayer?.() === true;
    return isPlayer
      && (typeof CurrentScreen === "undefined" || CurrentScreen === "ChatRoom")
      && settings.general.showHud
      && !settings.extreme.hideOwnIndicator
      && (this.state.trance || this.state.depth > 0);
  }

  private drawStatusIndicator(x: number, y: number, zoom: number): void {
    const ctx = MainCanvas;
    const depth = Math.max(0, Math.min(100, this.state.depth));
    const barX = x + 480 * zoom;
    const barY = y + 210 * zoom;
    const barW = Math.max(12, 24 * zoom);
    const barH = Math.max(100, 330 * zoom);
    const fillH = barH * depth / 100;
    const radius = Math.max(5, 9 * zoom);

    ctx.save();
    ctx.globalAlpha = 0.92;
    ctx.fillStyle = "rgba(8,3,15,.82)";
    ctx.strokeStyle = this.store.value.theme.primary;
    ...
    if (fillH > 0) {
      ...
      gradient.addColorStop(0, this.store.value.theme.secondary);
      gradient.addColorStop(.58, this.store.value.theme.primary);
      gradient.addColorStop(1, this.store.value.theme.accent);
      ctx.fillStyle = gradient;
      ctx.shadowColor = this.store.value.theme.primary;
      ctx.shadowBlur = Math.max(6, 14 * zoom);
      roundedRect(ctx, barX + 3 * zoom, barY + barH - fillH + 3 * zoom, Math.max(4, barW - 6 * zoom), Math.max(2, fillH - 6 * zoom), Math.max(2, radius - 3 * zoom));
      ctx.fill();
    }

    ctx.shadowBlur = 0;
    ctx.fillStyle = this.store.value.theme.accent;
    ctx.font = `700 ${Math.max(9, 14 * zoom)}px system-ui`;
    ctx.textAlign = "center";
    ctx.textBaseline = "bottom";
    ctx.fillText("SH", barX + barW / 2, barY - 6 * zoom);
    ctx.font = `600 ${Math.max(8, 11 * zoom)}px system-ui`;
    ctx.textBaseline = "top";
    ctx.fillText(`${Math.round(depth)}%`, barX + barW / 2, barY + barH + 5 * zoom);
    ctx.restore();
  }
}

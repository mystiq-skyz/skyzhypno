...
This repository contains the complete **0.1.1 implementation baseline**. It builds as:
...
Version **0.1.1** fixes the previous GitHub Pages CORS error. The loader no longer sets `script.crossOrigin = "anonymous"`. It first uses a normal classic script tag, then automatically retries through Tampermonkey's `GM_xmlhttpRequest`, with Raw GitHub as a second source if GitHub Pages cannot be reached.

# SkyzHypno 0.2.0 feature matrix
...
|---|---|---|
| Native Extensions registration | Implemented | Browser bootstrap verifies registration payload |
| Compact right-side character indicator | Implemented | Browser bootstrap invokes the DrawCharacter hook |
| Indicator hidden at 0% | Implemented | Browser bootstrap verifies overlay intensity is inactive at zero depth |
| Scrollable preferences and improved panel spacing | Implemented | CSS/static verification; live BC viewport check recommended |
| Extreme Mode quick-control lock | Implemented | Typecheck and UI logic |
| Remote indicator hide/show | Implemented | Protocol/schema tests and recipient permission checks |
| Implanted command instruction | Implemented | Suggestion schema test |
| Non-removable local emergency path | Implemented | Existing hypno/emergency tests |

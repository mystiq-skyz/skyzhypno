...
class ri {
  ...
  render() {
    ...
    e.innerHTML = `<div class="brand"><b>SH</b><span>SkyzHypno<small>v0.1.1</small></span></div><nav>${Ct.map((i) => `<button data-tab="${i}" class="${this.tab === i ? "active" : ""}">${At(i)}</button>`).join("")}</nav><button class="danger" data-action="emergency">Emergency Stop</button>`, t.innerHTML = `<div><h1>${this.remoteMember ? `Remote · ${q(this.runtime.characterName(this.remoteMember))}` : At(this.tab)}</h1><p>${ai(this.tab)}</p></div><button data-action="close">×</button>`, s.innerHTML = this.remoteMember ? this.renderRemoteTarget(this.remoteMember) : this.renderTab();
  }
  renderTab() {
    switch (this.tab) {
      case "overview":
...
class ui {
  ...
  constructor() {
    ...
    })), this.installSafetyControls(), this.network.broadcastHello(!0), this.runtime.localMessage("SkyzHypno v0.1.1 loaded.");
  }
  unload() {
